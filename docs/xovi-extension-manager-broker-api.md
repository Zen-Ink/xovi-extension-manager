...
```json
{
  ...
  "order": 50,
  "sourceEntryPath": "$XOVI_ROOT/exthome/example-extension/example-extension.so",
  ...
  "activeEntryTarget": "$XOVI_ROOT/exthome/example-extension/example-extension.so",
  "activeEntryConflict": "",
  "effectiveEnabled": true,
  "enabled": true,
  "hasManifest": true,
  "valid": true,
  "directoryPath": "$XOVI_ROOT/exthome/example-extension",
  "manifestPath": "$XOVI_ROOT/exthome/example-extension/manifest.json",
  "requires": {
    "xovi": ">=0.3.0",
    "extensions": {
...
- Disable is not uninstall. It only updates `manifest.enabled` and active
  symlinks; installed package files and data remain on disk.
- `remove` deletes managed packages and clears unmanaged legacy packages by
  moving their files/directories into `legacy-disabled`.
...
| `xovi-extension-manager$schema` | Return the current manifest schema summary exposed by the manager. | Ignored. Use an empty value. | Success: `{"ok":true,"manifestFiles":{"extension":"...","qmd":"..."},"installSupports":[".so",".qmd",".tar.gz",".tgz",".zip"],"required":["id"],"defaults":{...},"hardErrors":[...],"examples":{...}}`. | Command: `echo '>exovi-extension-manager$schema:' > /run/xovi-mb`<br>Sample response: `{"ok":true,"manifestFiles":{"extension":"$XOVI_ROOT/exthome/<id>/manifest.json",...},...}` |
