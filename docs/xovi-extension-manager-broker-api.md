...
| `xovi-extension-manager$repair` | Reconcile the active entry with `manifest.enabled`. Enabled packages receive a relative symlink; disabled packages have active symlinks removed, including QMD links left by an old order value. | Package `id` or package directory name. | Success includes `enabled`, `effectiveEnabled`, `actions`, `issues`, `requiresRestart`, and `restartTarget:"xochitl"`. Enable dependency checks still apply. An unknown non-symlink active entry is preserved and returns `{"ok":false,"error":"active-entry-conflict"}`. | Command: `echo '>exovi-extension-manager$repair:hide-dev-icon' > /run/xovi-mb` |
